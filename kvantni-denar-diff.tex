...
%DIF DEL kvantni-denar_old.tex   Sun Apr 30 20:13:14 2023
%DIF ADD kvantni-denar.tex       Sun Apr 30 20:18:20 2023
 \usepackage{amssymb}
\usepackage{amsmath}
\usepackage{amsthm}
\usepackage{float}
...
%DIFDELCMD <  %%%
\DIFdelend \DIFaddbegin \pagestyle{plain}
\setcounter{page}{1}
...
\begin{postulat}
(Bornovo pravilo) Pri meritvi opazljivke \DIFdelbegin \DIFdel{$A$ }\DIFdelend \DIFaddbegin \DIFadd{$A = \sum_{i = 1}^{n} \, \lambda_i \, | i \rangle \, \langle i |$ }\DIFaddend v stanju $| \psi \rangle$ bomo dobili rezultat $\lambda_j$ z verjetnostjo $p_j = | \langle j | \psi \rangle |^2$ \DIFaddbegin \DIFadd{za $j \in \{1,...,n\}$}\DIFaddend .
\end{postulat}

\begin{postulat}
(Kolaps valovne funkcije) Če pri meritvi opazljivke \DIFdelbegin \DIFdel{$A = \sum_i \, \lambda_i \, | i \rangle \, \langle i |$ izmerimo vrednost $\lambda_i$}\DIFdelend \DIFaddbegin \DIFadd{$A$ izmerimo vrednost $\lambda_j$}\DIFaddend , potem je takoj po meritvi sistem v lastnem stanju \DIFdelbegin \DIFdel{$| i \rangle$}\DIFdelend \DIFaddbegin \DIFadd{$| j \rangle$}\DIFaddend .
\end{postulat}
...
\end{equation}
%
kjer velja $\alpha$, $\beta \in \mathbb{C}$, $|\alpha|^2 + |\beta|^2 = 1$ in $| \psi \rangle$ normirana valovna funkcija. Zapis lahko dodatno poenostavimo, da velja $| 0 \rangle = [1,0]^T$ in $| 1 \rangle = [0,1]^T$, s čimer zapis kubita prevedemo na \DIFdelbegin \DIFdel{$| \psi \rangle = [\alpha, \beta]^T$}\DIFdelend \DIFaddbegin \DIFadd{$| \psi \rangle$ je $[\alpha, \beta]^T$}\DIFaddend .  Kubit zavzame stanje $| 0 \rangle$ ali $| 1 \rangle$ šele ob meritvi, in sicer z verjetnostjo $|\alpha|^2$ oz. $|\beta|^2$. \DIFdelbegin \DIFdel{Stanje, ko je $|\alpha|^2 = 0.5$ in $|\beta|^2 = 0.5$ včasih označimo tudi z $| + \rangle$ za pozitivno predznačen $\beta$ in $| - \rangle$ za negativno predznačen $\beta$}\DIFdelend \DIFaddbegin \DIFadd{Definirajmo še dve posebni stanji, ki jih bomo potrebovali v nadaljevanju, kot $| + \rangle = \frac{1}{\sqrt{2}}(|0\rangle + |1\rangle)$ in $| - \rangle = \frac{1}{\sqrt{2}}(|0\rangle - |1\rangle)$}\DIFaddend . \cite{nielsenQuantumComputationQuantum2012} \par Ker je valovna funkcija normirana in velja $|\alpha|^2 + |\beta|^2 = 1$, lahko $| \psi \rangle$ brez izgube splošnosti zapišemo tudi kot:
\begin{equation}
\label{blocheq}
...
Stanja večih kubitov zapišemo kot tenzorski produkt \DIFaddbegin \DIFadd{enokubitnih stanj}\DIFaddend . Za dva kubita: $| \psi \rangle_a = c_{a0} \, | 0 \rangle_a + c_{a1} \, | 1 \rangle_a$ in $| \psi \rangle_b = c_{b0} \, | 0 \rangle_b + c_{b1} \, | 1 \rangle_b$, tedaj velja
...
\end{split}
\end{equation}
kjer smo na primer stanje $ | 0 \rangle_a \otimes | 1 \rangle_b $ zapisali kot $ | 01 \rangle $. Enako kot smo to storili z enim kubitom, lahko stanja dveh kubitov predstavimo z vektorjem: $ | 00 \rangle = [1,0,0,0]^T $, $ | 01 \rangle = [0,1,0,0]^T $, $ | 10 \rangle = [0,0,1,0]^T $ in $ | 11 \rangle = [0,0,0,1]^T $. Poljubno stanje dveh kubitov zapišemo kot $ | \zeta \rangle = [c_{00}, c_{01}, c_{10}, c_{11}]^T $. Notacijo lahko posplošimo na poljubno število \DIFdelbegin \DIFdel{bitov. \mbox{%DIFAUXCMD
\cite{pompeKvantniAlgoritmi2022}
}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{kubitov. \mbox{%DIFAUXCMD
\cite{pompeKvantniAlgoritmiDiplomski2022}
}\hskip0pt%DIFAUXCMD
}\DIFaddend 

\par

Poljubnega stanja dveh ali več kubitov v splošnem \emph{ne} moremo zapisati \DIFdelbegin \DIFdel{v produktni bazi kot }\DIFdelend \DIFaddbegin \DIFadd{kot produktno stanje }\DIFaddend $ | \Psi \rangle_{ab} = | \psi \rangle_a \, |\phi \rangle_b$. Torej za dane koeficiente $c_{00}$, $c_{01}$, $c_{10}$ in $c_{11}$ ne moremo določiti koeficientov $c_{a0}$, $c_{a1}$, $c_{b0}$ in $c_{b1}$. Takšnemu stanju pravimo, da je \emph{prepleteno}. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{pompeKvantniAlgoritmi2022}
}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{pompeKvantniAlgoritmiDiplomski2022}
}\hskip0pt%DIFAUXCMD
}\DIFaddend \par \DIFdelbegin \DIFdel{Sistem z }\DIFdelend \DIFaddbegin \DIFadd{Sistemu }\DIFaddend $n$ \DIFdelbegin \DIFdel{kubiti ima }\DIFdelend \DIFaddbegin \DIFadd{kubitov ustreza }\DIFaddend $2^n$  \DIFdelbegin \DIFdel{možnih stanj}\DIFdelend \DIFaddbegin \DIFadd{dimenzijski Hilbertov prostor}\DIFaddend , torej je stanje le-tega superpozicija $2^n$ \DIFdelbegin \DIFdel{lastnih stanjoziroma ima vektor stanja $2^n$ komponent}\DIFdelend \DIFaddbegin \DIFadd{baznih stanj}\DIFaddend . Ko izmerimo stanje sistema, kolapsiramo superpozicijo in izmerimo \DIFdelbegin \DIFdel{le eno od $2^n$ možnih stanj s točno določeno verjetnostjo. \mbox{%DIFAUXCMD
\cite{pompeKvantniAlgoritmi2022}
}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{vse kubite hkrati. \mbox{%DIFAUXCMD
\cite{pompeKvantniAlgoritmiDiplomski2022}
}\hskip0pt%DIFAUXCMD
}\DIFaddend 

\subsubsection{Bellova stanja (pari EPR)}
Štiri specifična dvokubitna stanja, ki so med seboj maksimalno prepletena, imenujemo Bellova. Med seboj so ortogonalna in tvorijo bazo štiridimenzionalnega Hilbertovega prostora za dva kubita. Štiri Bellova stanja so: $|\Phi ^{+}\rangle = {\displaystyle { {(2)^{-1/2}(|00\rangle +|11\rangle )}}} $, $|\Phi ^{-}\rangle = {\displaystyle { {(2)^{-1/2}(|00\rangle -|11\rangle )}}}$, $|\Psi ^{+}\rangle = {\displaystyle { {(2)^{-1/2}(|01\rangle +|10\rangle )}}}$ in $|\Psi ^{-}\rangle = {\displaystyle { {(2)^{-1/2}(|01\rangle -|10\rangle )}}}$. Rezultat meritve enega samega kubita v enem izmed Bellovih stanj je naključen, vendar z meritvijo prvega kubita določimo tudi stanje, ki ga bomo izmerili pri \DIFdelbegin \DIFdel{drugi meritvi }\DIFdelend \DIFaddbegin \DIFadd{meritvi drugega}\DIFaddend . Velja, da bo \DIFdelbegin \DIFdel{druga meritev enaka }\DIFdelend \DIFaddbegin \DIFadd{rezultat druge meritve enak }\DIFaddend prvi za stanja $\Phi$ ali \DIFdelbegin \DIFdel{nasprotna prvemu }\DIFdelend \DIFaddbegin \DIFadd{nasproten prvi }\DIFaddend za stanja $\Psi$. To opažanje implicira, da sta rezultata meritev prvega in drugega kubita korelirana. Ime so dobila po Johnu Bellu, ki je prvi pokazal, da tako močne korelacije znotraj klasične fizike niso mogoče in da gre za pojave, ki jih lahko opazimo samo v svetu kvantne mehanike. \cite{BellState2023}
...
\end{equation}  
Jonesov vektor je tedaj definiran v ravnini $xy$ kot $ [ E_{0x}e^{i\phi _{x}}, E_{0y}e^{i\phi _{y}}]^T$. Vektorju pripadajo tipične polarizacije v \DIFdelbegin \DIFdel{Tabeli \ref{tab:pol}. }\DIFdelend \DIFaddbegin \DIFadd{tabeli \ref{tab:pol}. Valovno funkcijo posameznega fotona lahko tedaj opišemo na več načinov z različno izbiro baznih vektorjev. Primer iz tabele: $ { |R\rangle ={\frac{1}{\sqrt {2}}}{\big (}|H\rangle -i|V\rangle {\big )}} $). 
}\DIFaddend 
...
\DIFaddbegin \subsection{\DIFadd{Izbira baze za meritev}}

\par \DIFadd{Rezultat meritve polarizacije oz. spina je odvisen od izbire baze. Hkrati izbira meritvene baze predstavlja tudi teoretično osnovo za delovanje kvantnega denarja, zato vse skupaj fizikalno še malo bolje razložimo. Kot opisano v točki \ref{kubit}, je možno vsako stanje kubita reprezentirati s točko na Blochovi sferi kot prikazano na sliki \ref{fig:bloch}. 
}

\begin{figure}[H]
\includegraphics[width=6cm]{bloch.png}
\centering
\caption{\DIFaddFL{Blochova sfera. V smeri $z$ sta definirani stanji $| 0 \rangle$ in $| 1 \rangle$. \mbox{%DIFAUXCMD
\cite{BlochSphere2023}}\hskip0pt%DIFAUXCMD
}}
\label{fig:bloch}
\end{figure}

\DIFadd{V smeri $z$ definiramo stanji $| 0 \rangle$ in $| 1 \rangle$, ki predstavljata npr. spin dol in gor v smeri $z$. Vemo, da lahko spin merimo tudi v smeri $x$ in $y$, odvisno od orientacije Stern-Gerlachove naprave. Tedaj bi želeli zapisati tudi valovno funkcijo takšnega sistema. Pri tem si lahko pomagamo z enačbo \ref{blocheq} in vanjo vstavimo primerne kote $\theta$, $\varphi$ za bazo v smereh $x$, $y$ in $z$. Iz enačbe \ref{blocheq} tedaj dobimo 6 različnih normiranih valovnih funkcij (enačba \ref{eigenvectors}). Vemo, da ima spin dve lastni vrednosti, in sicer $\pm 1$. Tedaj so matrični operatorji, ki ustrezajo tem lastnim vrednostim in lastnim vektorjem, kar Paulijeve matrike $\sigma_x$, $\sigma_y$, $\sigma_z$. \mbox{%DIFAUXCMD
\cite{PauliMatrices2023}
}\hskip0pt%DIFAUXCMD
}

\begin{equation}
\DIFadd{\label{eigenvectors}
}{\DIFadd{\displaystyle }{\DIFadd{\begin{aligned}\psi _{x+}&={\frac{1}{\sqrt {2\,}}}{\begin{bmatrix}1\\1\end{bmatrix}}\;,&\psi _{x-}&={\frac{1}{\sqrt {2\,}}}{\begin{bmatrix}1\\-1\end{bmatrix}}\;,\\\psi _{y+}&={\frac{1}{\sqrt {2\,}}}{\begin{bmatrix}1\\i\end{bmatrix}}\;,&\psi _{y-}&={\frac{1}{\sqrt {2\,}}}{\begin{bmatrix}1\\-i\end{bmatrix}}\;,\\\psi _{z+}&={\begin{bmatrix}1\\0\end{bmatrix}}\;,&\psi _{z-}&={\begin{bmatrix}0\\1\end{bmatrix}}~.\end{aligned}}}}
\DIFadd{}\end{equation}

\par \DIFadd{Če torej naš kvantni sistem pripravimo v stanju $\psi_{x+}$ in merimo v smeri $x$, izmerimo lastno vrednost $+1$. V kolikor merimo ta isti sistem v smeri $z$ ali $y$, izmerimo lastno vrednost $+1$ z verjetnostjo $1/2$ in $-1$ z verjetnostjo $1/2$. Z drugimi besedami: ob meritvi dobimo enoličen rezultat le, če merimo v smeri lastnega vektorja valovne funkcije, torej moramo valovno funkcijo že od začetka poznati. 
}
...
\par
Da superluminalna komunikacija zares omogoča komunikacijo v preteklost, lahko potrdimo tudi matematično. Z \DIFdelbegin \DIFdel{$A - B$ }\DIFdelend \DIFaddbegin \DIFadd{$d = B - A$ }\DIFaddend označimo razdaljo med planetoma $A$ in $B$. Če signal potuje s hitrostjo $a$, je tedaj čas potovanja signala, merjen v inercialnem sistemu \DIFdelbegin \DIFdel{ko }\DIFdelend \DIFaddbegin \DIFadd{$S$, kjer }\DIFaddend sta točki A in B pri miru, enak
\begin{equation} \label{eq2}
\Delta t=t_{1}-t_{0}= \DIFdelbegin \DIFdel{\frac{B-A}{a}.
}\DIFdelend \DIFaddbegin \DIFadd{\frac{d}{a} = \frac{B - A}{a},
}\DIFaddend \end{equation}
\DIFaddbegin \DIFadd{kjer sta $t_0$ in $t_1$ časa oddaje in prejema signala v $S$.
}\DIFaddend Velja, da je dogodek \DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{A}$ }\DIFaddend (oddaja signala) vzrok dogodka \DIFdelbegin \DIFdel{B }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{B}$ }\DIFaddend (prejem signala). Za opazovalca v nekem drugem inercialnem sistemu \DIFaddbegin \DIFadd{$S'$}\DIFaddend , ki se giblje s hitrostjo $v$, lahko čas \DIFdelbegin \DIFdel{prejema signala v }\DIFdelend \DIFaddbegin \DIFadd{potovanja signala do }\DIFaddend B izračunamo s pomočjo Lorentzove transformacije\DIFdelbegin \DIFdel{, kjer je }\DIFdelend \DIFaddbegin \DIFadd{. Tedaj velja 
}\begin{equation} \DIFadd{\label{eq3}
}\begin{split}
\DIFadd{\Delta t' }& \DIFadd{=t'_{1}-t'_{0}=\frac{t_{1}-vB/c^{2}}{\sqrt{1-v^{2}/c^{2}}}-\frac{t_{0}-vA/c^{2}}{\sqrt{1-v^{2}/c^{2}}}}\\
 & \DIFadd{=\frac{1-av/c^{2}}{\sqrt{1-v^{2}/c^{2}}}\Delta t,
}\end{split}
\DIFadd{}\end{equation}
\DIFadd{kjer sta $t_0'$ in $t_1'$ časa oddaje in prejema signala merjena v $S'$ ter }\DIFaddend $c$ hitrost svetlobe. \DIFdelbegin \DIFdel{Tedaj velja 
}\begin{displaymath} \DIFdel{%DIFDELCMD < \label{eq3}%%%
}\begin{split}
\DIFdel{\Delta t' }& \DIFdel{=t'_{1}-t'_{0}=\frac{t_{1}-vB/c^{2}}{\sqrt{1-v^{2}/c^{2}}}-\frac{t_{0}-vA/c^{2}}{\sqrt{1-v^{2}/c^{2}}}}\\
 & \DIFdel{=\frac{1-av/c^{2}}{\sqrt{1-v^{2}/c^{2}}}\Delta t.
}\end{split}
\DIFdel{}\end{displaymath}%DIFAUXCMD
\DIFdelend Precej preprosto lahko pokažemo, da lahko za vrednosti $a > c$ izberemo tak $v$, da ima koeficient pred $\Delta t$ negativen predznak. To se zgodi že, če uporabimo \DIFdelbegin \DIFdel{$a = c^2 / (1 \, \frac{\text{m}}{\text{s}}) $ in $v = 2 \, \frac{\text{m}}{\text{s}}$}\DIFdelend \DIFaddbegin \DIFadd{$a = 4\cdot c$ in $v = \frac{1}{2} \cdot c$}\DIFaddend . Z drugimi besedami - v tem inercialnem sistemu se je dogodek \DIFdelbegin \DIFdel{B }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{B}$ }\DIFaddend zgodil še pred dogodkom \DIFdelbegin \DIFdel{A}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{A}$}\DIFaddend . Einstein je leta 1907 zaključil, da, čeprav v tem izrazu ne vidi nobene logične kontradikcije, je takšen rezultat kontradiktoren vsem našim izkušnjam iz realnega življenja. To pa v dovoljšnji meri dokazuje, da je primer, ko je $a > c$, nemogoč. \cite{TachyonicAntitelephone2022a}

\subsubsection{Komunikator FLASH}

\par Nick Herbert je leta 1982 \DIFaddbegin \DIFadd{vseeno }\DIFaddend predlagal shemo superluminalnega komunikatorja FLASH (\DIFdelbegin \DIFdel{Slika }\DIFdelend \DIFaddbegin \DIFadd{slika }\DIFaddend \ref{fig:flash}), katerega osnova je prepleten par fotonov (\DIFdelbegin \DIFdel{EPR par }\DIFdelend \DIFaddbegin \DIFadd{par EPR}\DIFaddend ). Za komunikacijo uporablja dejstvo, da ima meritev enega izmed prepletenih fotonov takojšnji učinek na drugega.
...
\label{fig:flash}
\end{figure}
\par Par prepletenih fotonov ustvarimo v dvofotonskem izviru in jih pošljemo v smeri dveh opazovalcev, npr. Alice in Bob. Stanje dveh \DIFdelbegin \DIFdel{fotnov je podano v rotacijsko invariantnem stanju
}\DIFdelend \DIFaddbegin \DIFadd{fotonov je podano z
...
\end{equation} 
Laserska \DIFdelbegin \DIFdel{fotopomnoževalka }\DIFdelend \DIFaddbegin \DIFadd{pomnoževalka }\DIFaddend v tem primeru \DIFdelbegin \DIFdel{dejansko pomnoži stanja }\DIFdelend \DIFaddbegin \DIFadd{iz $n \cdot |H\rangle$ stanj ustvari $4n$ kopij stanj }\DIFaddend $| R \rangle$ \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{ali }\DIFaddend $| L \rangle$, \DIFdelbegin \DIFdel{vsakega }\DIFdelend \DIFaddbegin \DIFadd{vsako }\DIFaddend z verjetnostjo $1/2$. Torej na izhodu dobimo $2n$ fotonov v stanju $| R \rangle$ in $2n$ fotonov v stanju $| L \rangle$. Rezultat Bobovih meritev bi bil tedaj vedno (in neodvisno od originalne polarizacije, ki jo izmeri Alice) enak $n/2$ fotonov za vse 4 polarizacije H, V, L in R. \cite{marcocerezoEntangledParticlesFaster2015}
...
%DIFDELCMD < \par %%%
\DIFdel{Kaj pomeni izbira baze, lahko fizikalno še malo bolje razložimo. Kot opisano v točki \ref{kubit}, je možno vsak kubit reprezentirati s točkami na Blochovi sferi kot prikazano na Sliki \ref{fig:bloch}. }%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[H]
%DIFDELCMD < \includegraphics[width=6cm]{bloch.png}
%DIFDELCMD < \centering
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Blochova sfera. V smeri $z$ sta definirani stanji $| 0 \rangle$ in $| 1 \rangle$. \mbox{%DIFAUXCMD
\cite{BlochSphere2023}}\hskip0pt%DIFAUXCMD
}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:bloch}
...
%DIFDELCMD < %%%
\begin{displaymath}
\DIFdel{%DIFDELCMD < \label{eigenvectors}%%%
}{\DIFdel{\displaystyle }{\DIFdel{\begin{aligned}\psi _{x+}&={\frac{1}{\sqrt {2\,}}}{\begin{bmatrix}1\\1\end{bmatrix}}\;,&\psi _{x-}&={\frac{1}{\sqrt {2\,}}}{\begin{bmatrix}1\\-1\end{bmatrix}}\;,\\\psi _{y+}&={\frac{1}{\sqrt {2\,}}}{\begin{bmatrix}1\\i\end{bmatrix}}\;,&\psi _{y-}&={\frac{1}{\sqrt {2\,}}}{\begin{bmatrix}1\\-i\end{bmatrix}}\;,\\\psi _{z+}&={\begin{bmatrix}1\\0\end{bmatrix}}\;,&\psi _{z-}&={\begin{bmatrix}0\\1\end{bmatrix}}~.\end{aligned}}}}
\DIFdel{}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < \par %%%
\DIFdel{Če torej naš kvantni sistem pripravimo v stanju $\psi_{x+}$ in merimo v smeri $x$, }\DIFdelend \DIFaddbegin \DIFadd{Do te številke lahko za sistem z dvema meritvenima bazama intuitivno pridemo na sledeč način. Ponarejevalec lahko z verjetnostjo $(1/2)$ ugane pravo bazo. Velja pa tudi, da je verjetnost, da pri napačno izbrani bazi pravilno }\DIFaddend izmerimo lastno vrednost\DIFdelbegin \DIFdel{$+1$. V kolikor merimo ta isti sistem v smeri $z$ ali $y$, izmerimo lastno vrednost $+1$ z verjetnostjo $1/2$ in $-1$ z verjetnostjo $1/2$. Z drugimi besedami: ob meritvi dobimo enoličen rezultat le, če merimo v smeri lastnega vektorja valovne funkcije, torej moramo valovno funkcijo že od začetka poznati}\DIFdelend \DIFaddbegin \DIFadd{, enaka $(1/2)^2$. Sledi, da je $(1/2) + (1/2)^2 = (3/4)$}\DIFaddend .
...
Trenutno ne obstaja praktična tehnologija, ki bi omogočala obstoj kvantnega denarja. Za to namreč potrebujemo t.i. kvantni pomnilnik - komponento analogno npr. pomnilniku FLASH - ki je za daljše časovno obdobje sposobna shraniti stanje kubita in je odporna na motnje iz okolice. Trenutno je to eno izmed bolj aktivnih področij raziskav. Avtorjem nedavnega članka \cite{RobustMultiqubitQuantum} je uspelo ustvariti dvokubitni register, ki lahko pri temperaturi 1.5 \DIFdelbegin \DIFdel{$K$ }\DIFdelend \DIFaddbegin \DIFadd{$\text{K}$ }\DIFaddend shrani kvantno stanje fotonov za nekaj več kot 2 sekundi.  

\section{Zaključek}

V okviru tega seminarja smo preleteli osnovne koncepte kvantne informatike, dokazali izrek o prepovedi kloniranja in pokazali njegovo uporabo na konceptu kvantnega denarja, ki rešuje praktičen problem ponarejanja. Poleg kvantnega denarja so aktivna področja raziskav tudi: super gosto kodiranje \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{SuperdenseCoding2023a}}\hskip0pt%DIFAUXCMD
}\DIFaddend , kvantna izmenjava ključev (QKD) \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{QuantumKeyDistribution2023a}}\hskip0pt%DIFAUXCMD
}\DIFaddend , kvantne kode za popravljanje napak \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{QuantumErrorCorrection2023}}\hskip0pt%DIFAUXCMD
}\DIFaddend , kvantni algoritmi, itd. \DIFdelbegin \DIFdel{Zanimvo }\DIFdelend \DIFaddbegin \DIFadd{Zanimivo }\DIFaddend bo videti, kako hitro nam bo uspelo zapletene kvantne naprave iz laboratorijev preseliti v naš vsakdan.
