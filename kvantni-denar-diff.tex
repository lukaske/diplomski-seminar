...
%DIF ADD kvantni-denar.tex       Thu Apr 20 15:17:14 2023
 \usepackage{amssymb}
\usepackage{amsmath}
\usepackage{amsthm}
\usepackage{float}
...
\DIFadd{S tem smo postavili osnove matematičnega formalizma, s katerim bomo delali v nadaljevanju. \mbox{%DIFAUXCMD
...
\DIFdelbegin \subsubsection{\DIFdel{Bellova stanja (pari EPR)}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdel{Štiri specifična dvokubitna stanja imenujemo Bellova stanja. Za ta stanja je značilno, da so med seboj maksimalno prepletena in hkrati tvorijo bazo štiri-dimenzijskega Hilbertovega prostora za dva kubita. Štiri Bellova stanja so:
...
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdel{Rezultat meritve enega samega kubita v enem izmed Bellovih stanj je naključen, vendar z meritvijo prvega kubita določimo tudi stanje, ki ga bomo izmerili pri drugi meritvi. Velja, da bo druga meritev enaka prvi za stanja $\Phi$ ali nasprotna prvemu za stanja $\Psi$. To opažanje implicira, da sta rezultata meritev prvega in drugega kubita korelirana. Stanja se imenujejo Bellova, saj je John Bell prvi pokazal, da tako močne korelacije znotraj klasične fizike niso mogoče in da gre za pojave, ki jih lahko opazimo samo v svetu kvantne mehanike. \mbox{%DIFAUXCMD
\cite{BellState2023}
}\hskip0pt%DIFAUXCMD
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Polarizacija svetlobe}
